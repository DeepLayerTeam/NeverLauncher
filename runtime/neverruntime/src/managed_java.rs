...
pub async fn ensure_managed_java(
    ...
    if !matches!(required_major, 8 | 17 | 21 | 25) {
        return Err(format!(
            "Managed Java 0.10.4 поддерживает Java 8/17/21/25; запрошена Java {required_major}"
        ));
    }
...
fn normalize_distribution(value: &str) -> Result<String, String> {
    match value.trim().to_ascii_lowercase().as_str() {
        ...
        other => Err(format!("Managed Java distribution {other} не поддерживается в 0.10.4")),
    }
}
