...
func (s Server) registerAuthRoutesV1(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /api/v1/auth/providers/{providerId}/link/begin", s.authProviderLinkBegin0120)
	mux.HandleFunc("POST /api/v1/auth/providers/{providerId}/link/complete", s.authProviderLinkComplete0120)
	...
	mux.HandleFunc("GET /api/v1/auth/devices", s.authDevices0125)
	mux.HandleFunc("GET /api/v1/auth/device-trust", s.authDeviceTrustStatus0121)
	mux.HandleFunc("POST /api/v1/auth/devices/register/begin", s.authDeviceRegisterBegin0121)
	mux.HandleFunc("POST /api/v1/auth/devices/register/complete", s.authDeviceRegisterComplete0121)
	mux.HandleFunc("POST /api/v1/auth/devices/{deviceId}/verify/begin", s.authDeviceVerifyBegin0121)
	...
	mux.HandleFunc("POST /api/v1/auth/devices/revoke-others", s.authDeviceRevokeOthers0125)
	mux.HandleFunc("POST /api/v1/auth/devices/key-rotation/begin", s.authDeviceKeyRotateBegin0128)
	mux.HandleFunc("POST /api/v1/auth/devices/{deviceId}/key-rotation/complete", s.authDeviceKeyRotateComplete0128)
	mux.HandleFunc("POST /api/v1/auth/devices/key-recovery/begin", s.authDeviceKeyRecoverBegin0128)
	mux.HandleFunc("POST /api/v1/auth/devices/{deviceId}/key-recovery/complete", s.authDeviceKeyRecoverComplete0128)
	mux.Handle("GET /api/v1/admin/auth/sessions", s.requirePermission("users:manage", s.adminAuthSessions118))
	mux.Handle("GET /api/v1/admin/auth/devices", s.requirePermission("users:manage", s.adminAuthDevices0125))
	mux.Handle("DELETE /api/v1/admin/auth/devices/{deviceId}", s.requireFreshAuth117("users:manage", "phishing-resistant", 5*time.Minute, s.adminAuthDeviceRevoke0125))
	mux.Handle("POST /api/v1/admin/auth/devices/{deviceId}/revoke", s.requireFreshAuth117("users:manage", "phishing-resistant", 5*time.Minute, s.adminAuthDeviceRevoke0125))
	mux.Handle("GET /api/v1/admin/auth/federation/status", s.requirePermission("users:manage", s.authFederationStatus0120))
