...
func TestPublishSignedPublishesVerifiedManifestAtomically(t *testing.T) {
	...
	_, err := repo.CreateVersion("demo-project", "vanilla", "stable", "0.10.3-test")
	if err != nil {
		t.Fatal(err)
	}
	releaseID := "demo-project-vanilla-0.10.3-test"
	_, err = repo.AddFile(model.FileObject{
		...
		URL:       "http://example.test/api/v1/files/demo-project/0.10.3-test/libraries/fixture.jar",
		Required:  true,
	})
	if err != nil {
		t.Fatal(err)
	...
	published, err := s.publishSigned("demo-project", "vanilla", "stable", "0.10.3-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestPublishSignedDoesNotPublishWhenSigningFails(t *testing.T) {
	repo := repository.NewMemoryRepository("http://example.test")
	_, err := repo.CreateVersion("demo-project", "vanilla", "stable", "0.10.3-invalid-signing")
	if err != nil {
		t.Fatal(err)
	}
	s := Server{Config: config.Config{Environment: "production", ManifestSigningPrivateKey: "not-a-key"}, Repo: repo, State: NewRuntimeState()}
	if _, err = s.publishSigned("demo-project", "vanilla", "stable", "0.10.3-invalid-signing"); err == nil {
	...
	for _, item := range versions {
		if item.Version == "0.10.3-invalid-signing" {
