...
func installForgeLike(ctx context.Context, opts forgeMaterializeOptions) (forgeMaterializeResult, error) {
	...
	if bundle.Profile.JSON == "" && bundle.Profile.Version == "" {
		return forgeMaterializeResult{}, fmt.Errorf("%s installer profile не содержит version/json metadata; legacy pre-1.13 installer format в 0.10.5 не поддерживается", loader)
	}
