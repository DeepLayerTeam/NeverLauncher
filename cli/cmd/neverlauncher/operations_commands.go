...
func resolveLoaderMetadata(loader, minecraftVersion, loaderVersion, metadataPath, installerProfile string) (LoaderMetadata, string, error) {
	...
	return LoaderMetadata{}, "", fmt.Errorf("loader %s требует материализованный profile metadata или --installer-profile; builtin fallback metadata в 0.10.5 запрещены", loader)
}
...
func loaderCompatibility(loader string) map[string]any {
	return map[string]any{
		"java":                   []int{17, 21},
		"minecraftRange":         "Fabric/Quilt по официальному Meta API; Forge processor-based installers 1.13+; NeoForge processor-based installers; legacy Forge pre-1.13 пока вне 0.10.5",
