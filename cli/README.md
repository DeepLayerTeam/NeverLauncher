...
Рабочие materializer-команды `0.10.5`:
...
Fabric использует официальный Meta API v2, Quilt — Meta API v3. Forge и NeoForge загружают проверенный official Maven installer JAR, выполняют client processors из `install_profile.json`, проверяют outputs и нормализуют дочерний `version.json`. До упаковки mutable alias `latest-stable` разрешается в конкретную loader version, а все runtime artifacts фиксируются обычным SHA-256 + signed manifest lifecycle NeverLauncher. Legacy Forge pre-1.13 в 0.10.5 не объявляется поддерживаемым.
...
```bash
nl release doctor
nl release build --out dist/release-0.10.5
nl release sign dist/release-0.10.5 --private-key /secure/release-private.pem
nl release verify dist/release-0.10.5 --public-key /etc/neverlauncher/release-public.pem
...
```bash
nl desktop package --artifact-dir dist/release-0.10.5 --out dist/desktop-package --platform linux
...
nl security provenance --source-root . --artifact-dir dist/release-0.10.5 --output PROVENANCE.json
```
