...
RUNTIME_DIR="$ROOT/e2e/serverbridge-ha-migration-runtime"
RESULT_DIR="$ROOT/e2e/serverbridge-ha-migration-result"
...
PROJECT_NAME="neverlauncher-serverbridge-ha-migration"
...
need(){ command -v "$1" >/dev/null 2>&1 || { echo "[serverbridge-ha-migration] required command missing: $1" >&2; exit 1; }; }
...
for migration in "$ROOT"/services/api/internal/dbmigrate/sql/*.sql; do
  base="$(basename "$migration")"; ordinal="${base%%_*}"; (( 10#$ordinal > 28 )) && continue
...
latest_before="$(psql "$DB_DSN" -Atqc 'SELECT max(version) FROM schema_migrations')"; [[ "$latest_before" == "0028_zero_patch_topology_handoff_0148" ]]
psql "$DB_DSN" -v ON_ERROR_STOP=1 <<'SQL' >/dev/null
INSERT INTO projects(id,name,default_channel) VALUES ('ha-project','HA Project','stable');
...
VALUES
 ('proxy-ha','proxy-ha','velocity','ha-project','vanilla','','','','active',2,'ed25519',repeat('A',43),repeat('a',64),1,now(),now(),now()),
 ('backend-ha','backend-ha','paper','ha-project','vanilla','','','','active',2,'ed25519',repeat('B',43),repeat('b',64),1,now(),now(),now());
...
VALUES('proxy-ha','backend-ha','backend-ha','ha-project','vanilla','active',now(),now());
INSERT INTO server_bridge_node_nonces_v2(node_id,nonce_hash,identity_epoch,consumed_at,expires_at)
VALUES('proxy-ha',repeat('c',64),1,now()-interval '4 minutes',now()-interval '1 minute');
SQL
...
latest_after="$(psql "$DB_DSN" -Atqc 'SELECT max(version) FROM schema_migrations')"; [[ "$latest_after" == "0029_serverbridge_public_matrix_ha_hardening_0149" ]]
[[ "$(psql "$DB_DSN" -Atqc "SELECT count(*) FROM server_bridge_nodes_v2 WHERE id IN ('proxy-ha','backend-ha')")" == 2 ]]
[[ "$(psql "$DB_DSN" -Atqc "SELECT count(*) FROM server_bridge_topology_edges_v2 WHERE source_node_id='proxy-ha' AND target_node_id='backend-ha'")" == 1 ]]
for index in idx_server_bridge_nodes_active_heartbeat_0149 idx_server_bridge_topology_active_freshness_0149 idx_server_bridge_handoffs_active_expiry_0149 idx_server_bridge_join_active_expiry_0149; do
