...
RUNTIME_DIR="$ROOT/e2e/bukkit-family-migration-runtime"
RESULT_DIR="$ROOT/e2e/bukkit-family-migration-result"
...
PROJECT_NAME="neverlauncher-bukkit-family-migration"
...
need() { command -v "$1" >/dev/null 2>&1 || { echo "[bukkit-family-migration] required command missing: $1" >&2; exit 1; }; }
...
[[ "$latest_before" == "0023_one_time_join_tickets_0143" ]]

psql "$DB_DSN" -v ON_ERROR_STOP=1 <<'SQL' >/dev/null
INSERT INTO projects(id,name,default_channel) VALUES ('bukkit-family-project','Bukkit Family Project','stable');
INSERT INTO server_bridge_nodes_v2(
  ...
  'paper-before-0144','Paper Before 0144','paper','bukkit-family-project','vanilla','','','','active',2,
...
[[ "$latest_after" == "0024_bukkit_family_0144" ]]
...
for kind in bukkit spigot paper purpur folia; do
  ...
  psql "$DB_DSN" -v ON_ERROR_STOP=1 -v id="${kind}-0144" -v name="${kind} 0144" -v kind="$kind" -v fingerprint="$fingerprint" <<'SQL' >/dev/null
INSERT INTO server_bridge_nodes_v2(
  id,name,kind,project_id,profile_id,fingerprint,token_hash,token_prefix,status,protocol_version,
  key_algorithm,public_key,key_fingerprint,identity_epoch,identity_rotated_at,created_at
) VALUES (
  :'id',:'name',:'kind','bukkit-family-project','vanilla','','','','active',2,
...
INSERT INTO server_bridge_nodes_v2(
  ...
  'invalid-0144','Invalid 0144','craftbukkit-plus','bukkit-family-project','vanilla','','','','active',2,
...
family_count="$(psql "$DB_DSN" -Atqc "SELECT count(*) FROM server_bridge_nodes_v2 WHERE kind IN ('bukkit','spigot','paper','purpur','folia')")"
(( family_count >= 6 )) || { echo "[bukkit-family-migration] expected family nodes, got $family_count" >&2; exit 1; }
